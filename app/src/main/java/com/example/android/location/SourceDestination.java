...
public class SourceDestination extends AppCompatActivity implements OnMapReadyCallback {
...
//        m_map.setOnIndoorStateChangeListener((GoogleMap.OnIndoorStateChangeListener) this);
    ...
    void addpoly() {
      /*  if(sourceLatLng!=null && destinationLatLng!=null)
        m_map.addPolyline(new PolylineOptions().add(sourceLatLng).add(destinationLatLng)).setColor(Color.RED);*/
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        if (requestCode == PLACE_AUTOCOMPLETE_REQUEST_CODE) {
            ...
            if (resultCode == RESULT_OK) {
                destination = PlaceAutocomplete.getPlace(this, intent);
                destinationLatLng = destination.getLatLng();
                destinationEditText.setText(destination.getName());
                if (flag2 == true)
         ...
         /*   if(source == null)
            {
                ...
                if (markerLatLng.toString().equals(sourceLatLng.toString())){
                    if(marker.getTitle().equals("Your Location"))
                    {
                       ...
                       // Log.v("checkHere","sourceAddress"+source.getAddress());
                        ...
                        intent.putExtra("placerating","");
                        intent.putExtra("placeprice","");
                    }
                    else {
                        Log.v("checkHere", "sourcelatlng" + source.getLatLng());
                        ...
                        intent.putExtra("placewebsite", source.getWebsiteUri());
                ...
                if (markerLatLng.toString().equals(destinationLatLng.toString())) {
                    ...
                    intent.putExtra("placewebsite",destination.getWebsiteUri());
